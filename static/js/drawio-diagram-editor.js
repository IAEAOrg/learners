 ...
 */
DiagramEditor.prototype.frameStyle = 'position:absolute;border:0;width:100vw;height:100vh;';
...
DiagramEditor.prototype.getElementData = function(elem)
{
    ...
    if (elem.contentDocument) {
        let svgContent = elem.contentDocument.documentElement.outerHTML;
...
DiagramEditor.prototype.startEditing = function(data, format, title)
{
	if (this.frame == null)
	{
		...
		this.frame = this.createFrame(
			this.getFrameUrl(),
			this.getFrameStyle());
        document.getElementById("drawio-container").appendChild(this.frame);
...
DiagramEditor.prototype.setActive = function(active)
{
	if (active)
	{
        ...
        document.getElementById("drawio-container").style.opacity = 1;
        document.getElementById("drawio-container").style.pointerEvents = 'all';
	}
	else
	{
        document.body.style.overflow = this.previousOverflow;
        document.getElementById("drawio-container").style.opacity = 0;
        document.getElementById("drawio-container").style.pointerEvents = 'none';
	}
};
...
DiagramEditor.prototype.stopEditing = function()
{
	if (this.frame != null)
	{
		...
		document.getElementById("drawio-container").removeChild(this.frame);
